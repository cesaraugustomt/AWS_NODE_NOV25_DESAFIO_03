...
export class CarsRepository {
  ...
  public async create({
    ...
    const car_items = items.map((item) => {
      return { name: item };
    });
  ...
  public async update(
    car: Car,
    { plate, brand, model, km, year, price, status, items }: UpdateCarDTO
  ): Promise<Car> {
    const car_items = items?.map((item) => {
      return { name: item };
    });
  ...
  public async findAll({
    ...
    // Filters
    if (status) queryBuilder.andWhere("car.status = :status", { status });

    if (brand) queryBuilder.andWhere("car.brand = :brand", { brand });

    if (model) queryBuilder.andWhere("car.model = :model", { model });
    ...
    if (km) queryBuilder.andWhere("car.km <= :km", { km });

    if (fromYear) queryBuilder.andWhere("car.year >= :fromYear", { fromYear });

    if (untilYear)
      queryBuilder.andWhere("car.year <= :untilYear", { untilYear });

    if (minPrice) queryBuilder.andWhere("car.price >= :minPrice", { minPrice });

    if (maxPrice) queryBuilder.andWhere("car.price <= :maxPrice", { maxPrice });
    ...
    const cars = await this.ormRepository
      ...
      .orderBy(orderBy ? `car.${orderBy}` : "car.id", orderDirection || "ASC")
      .getMany();
